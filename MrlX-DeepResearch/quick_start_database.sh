#!/bin/bash

echo "=========================================="
echo "  Database Server Quick Start"
echo "=========================================="

# Get script directory
QUICKSTART_ROOT="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

# Check if .env file exists
if [ ! -f "$QUICKSTART_ROOT/.env" ]; then
    echo "Error: .env file not found"
    echo ""
    echo "Please follow these steps:"
    echo "1. Copy template: cp env.example .env"
    echo "2. Edit .env file and fill in all required variables"
    exit 1
fi

# Initialize Database Server environment variables
echo ""
echo "Step 1/3: Initializing Database Server environment variables..."
source "$QUICKSTART_ROOT/init_env/init_database_server_env.sh"

# Check if database server is already running
echo ""
echo "Step 2/3: Checking if Database Server is already running..."
DATABASE_PORT=${DATABASE_SERVER_PORT:-18888}

if command -v lsof &> /dev/null; then
    if lsof -Pi :$DATABASE_PORT -sTCP:LISTEN -t >/dev/null 2>&1; then
        echo "Warning: Database Server is already running on port $DATABASE_PORT"
        echo "If you want to restart, please stop the existing process first:"
        echo "  lsof -ti:$DATABASE_PORT | xargs kill -9"
        exit 0
    fi
elif command -v netstat &> /dev/null; then
    if netstat -tuln 2>/dev/null | grep -q ":$DATABASE_PORT "; then
        echo "Warning: Database Server is already running on port $DATABASE_PORT"
        echo "If you want to restart, please stop the existing process first"
        exit 0
    fi
else
    echo "Warning: Cannot detect if port is in use (lsof/netstat not available)"
fi

# Start Database Server
echo ""
echo "Step 3/3: Starting Database Server..."
cd "$QUICKSTART_ROOT/db"

# Create logs directory if not exists
mkdir -p "$QUICKSTART_ROOT/logs"

# Start database server in background
nohup python database_server.py > "$QUICKSTART_ROOT/logs/database_server.log" 2>&1 &
DATABASE_PID=$!
echo "Database Server started with PID: $DATABASE_PID"

# Wait for server to be ready (check health endpoint)
echo "Waiting for Database Server to be ready..."
MAX_WAIT=300
WAIT_COUNT=0

while [ $WAIT_COUNT -lt $MAX_WAIT ]; do
    if curl -s http://localhost:$DATABASE_PORT/health >/dev/null 2>&1; then
        echo "Database Server is ready!"
        echo "=========================================="
        echo "Database Server started successfully"
        echo "  - Host: $DATABASE_SERVER_HOST"
        echo "  - Port: $DATABASE_PORT"
        echo "  - PID: $DATABASE_PID"
        echo "  - Logs: $QUICKSTART_ROOT/logs/database_server.log"
        echo "=========================================="
        exit 0
    fi

    if [ $((WAIT_COUNT % 10)) -eq 0 ]; then
        echo "Waiting for Database Server to be ready... ($WAIT_COUNT/$MAX_WAIT seconds)"
    fi
    sleep 1
    WAIT_COUNT=$((WAIT_COUNT + 1))
done

echo ""
echo "Warning: Database Server may not be fully ready yet"
echo "Check logs: tail -f $QUICKSTART_ROOT/logs/database_server.log"
